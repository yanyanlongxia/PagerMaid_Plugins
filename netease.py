# -*- coding: utf-8 -*-
import json, requests, re, base64, codecs, random, math
...
async def nem(context, args, origin_text):
    ...
    proxy = [{'http': 'http://192.210.137.108:8080', 'https': 'http://192.210.137.108:8080'},
             {'http': 'http://music.lolico.me:39000', 'https': 'http://music.lolico.me:39000'},
             {'http': 'http://aimer.one:2333',
              'https': 'http://aimer.one:2333'},
             {'http': 'http://fs2.ilogic.net.cn:5200', 'https': 'http://fs2.ilogic.net.cn:5200'},
             {'http': 'http://64.64.250.246:8080', 'https': 'http://64.64.250.246:8080'}]
            ...
            for _ in range(20):  # 最多重试20次
                ...
                try:
                    ...
                    if randsong.status_code == 200:
                        ...
                        if apinum == 0 and randsong['success']:
                            parameter[0] = "id"
                            parameter.append(
    ...
    keyword = keyword[:-1]
    idplay = False
    if parameter[0] == "id":  # ID点歌功能
        if len(parameter) > 2:
            await context.edit(helptext)
            return
        idplay = keyword
        parameter[0] = "p"
    if parameter[0][0] == "s":  # 搜索功能
        await context.edit(f"【{keyword}】搜索中 . . .")
        if len(parameter[0]) > 1:
            limit = str(parameter[0][1:])
        else:
            limit = "5"
        ...
        for _ in range(20):  # 最多尝试20次
            ...
            if req.status_code == 200:
                ...
                if result:
                    ...
                    await context.edit(text, parse_mode='html')
                    status = True
                    break
                else:
        ...
        if reply:
            ...
            if search:
                try:
                    ...
                    if reply.from_user.id == context.from_user.id:
        ...
        try:
            ...
            imported = False
            await bot.send_message(context.chat.id,
                ...
                if result:
                    ...
                    if ccimported is False:  # 下载(普通音质)
                        music = requests.request(
                            "GET",
                            "http://music.163.com/api/song/enhance/download/url?&br=" + str(info['br']) + "&id=" + str(
                                info['id']), headers=headers, proxies=proxies, verify=False)
                        if music.status_code == 200:
                            music = json.loads(music.content)
                            if not music['data']['url']:
                                music = requests.request(
                                    "GET", "https://music.163.com/song/media/outer/url?id=" + str(info['id']) + ".mp3",
