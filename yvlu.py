...
async def yv_lu(context):
    ...
    for num in range(1, 5):
        if not exists('plugins/yvlu/p' + str(num) + '.png'):
            re = get('https://raw.githubusercontent.com/yanyanlongxia/PagerMaid_Plugins/master/yvlu/p' + str(num) + '.png')
            with open('plugins/yvlu/p' + str(num) + '.png', 'wb') as bg:
                bg.write(re.content)
    if not exists('plugins/yvlu/mask.png'):
        re = get('https://raw.githubusercontent.com/yanyanlongxia/PagerMaid_Plugins/master/yvlu/mask.png')
        with open('plugins/yvlu/mask.png', 'wb') as bg:
            bg.write(re.content)
    if not exists('plugins/yvlu/mask1.png'):
        re = get('https://raw.githubusercontent.com/yanyanlongxia/PagerMaid_Plugins/master/yvlu/mask1.png')
        with open('plugins/yvlu/mask1.png', 'wb') as bg:
            bg.write(re.content)
    if not exists('plugins/yvlu/ZhuZiAWan-2.ttc'):
        await context.edit('下载字体中。。。')
        re = get('https://raw.githubusercontent.com/yanyanlongxia/Telegram_PaimonBot/master/assets/fonts/ZhuZiAWan-2.ttc')
